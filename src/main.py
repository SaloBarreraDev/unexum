...
class Widget_Principal(ScreenManager):
    ...
    def dialogo_sesion(self, *args):
        if self.sesion_activa():
            def on_datos(datos):
                ...
                MDDialogSupportingText(
                    text=f"Bienvenido, {datos["name"]}.\n{("Tu email está verificado." if datos["emailVerification"] else "Tu email no está verificado.")}",
                    markup=True,
                    halign="center",
                ),
                size_hint_x=0.95,
            ...
            self.buscar_datos_cuenta(on_datos)
        else:
            app = MDApp.get_running_app()
    ...
    def iniciar_sesion(self, email, password, dlg):
        ...
        def exito(req, result, dlg):
            ...
            if cookie_header:
                ...
                Logger.info("Login Exitoso. Token guardado.")
                self.cerrar_dialogo(dlg)
            else:
                Logger.info("Fallo crítico: Cookie no encontrada.")
    ...
    def buscar_datos_cuenta(self, callback):
        if not STORE.exists('auth'):
            return
        url = f"{BASE_URL}/account"
        def exito(req, result): return callback(result)
        def fallo(req, result): 
    ...
    def cerrar_sesion(self, dlg):
        if not STORE.exists('auth'): return
    ...
    def descargar_documento(self, url, nombre_archivo, unidad, peso):
        ...
        def exito(req, result):
            MDSnackbar(MDSnackbarText(text='Archivo descargado correctamente.', halign="left"),
                duration=5, pos_hint={"center_x": 0.5}, y=dp(5), size_hint_x=0.8).open()
            if platform == "android":
                app.ss.copy_to_shared(nombre_destino, filepath=join("Repositorio", self.pantalla_listado.materia, unidad, nombre_archivo))
    ...
    def ir_a_upload(self):
        ...
        def recibir_datos(datos_cuenta):
            if not datos_cuenta["emailVerification"]:
        ...
        self.buscar_datos_cuenta(recibir_datos)
    ...
    def seleccionar_archivo(self):
        ruta = None
        if platform == "win":
            ruta = r"C:\Users\Salomon\Desktop\Unexum\unexum\Examen de Simulación de Inv. de Ope. II Alejandro. Dirimo. Salomon. Leonardo-FINAL.docx"
    ...
    def _subida_exitosa(self, resultado, nombre_archivo, dialogo):
        ...
        print(f"Archivo físico subido al Bucket. ID asignado: {file_id}")
    ...
    def crear_registro_bd(self, file_id, nombre_archivo):
        ...
        def exito(req, result):
            MDSnackbar(MDSnackbarText(text='¡Archivo subido al repositorio!', halign="left"),
                duration=5, pos_hint={"center_x": 0.5}, y=dp(5), size_hint_x=0.8).open()
        def fallo(req, result):
